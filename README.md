...
1. Navigate to `D:\Users\alper` → `dp-cvep` → `cvep_speller_env` → `dp-lsl-recording` → `configs`.
...
```text
D:\Users\alper\                         → Persistent Lab Drive (Deep Freeze Safe)
├── Miniconda3/                     → Python ecosystem
├── Git/                            → Version control
├── msys64/                         → C Compiler Ecosystem
│   └── ucrt64/
│       ├── bin/                      → Contains GCC, SDL3.dll, and lsl.dll (Runtime DLL)
│       ├── lib/                      → Contains liblsl.dll.a (Link-time Library)
│       └── include/                  
│           ├── lsl_c.h               → LSL C Header (Copied manually in Step 4)
│           └── lsl/                  → LSL Additional Headers (Copied manually in Step 4)
└── dp-cvep/                          → Master Dareplane Setup Repository (Cloned in Step 2)
    ├── setup_cvep_demo_biosemi.py
    └── cvep_speller_env/               → Auto-generated by the setup script
        ├── data/                       → EEG Data and decoded logs output folder
        ├── dp-control-room/            → Dareplane Web UI
        ├── dp-cvep-decoder/            → Python rCCA Prediction AI
        ├── dp-lsl-recording/           → LSL XDF Data Recorder
        └── dp-cvep-speller/            → THIS REPOSITORY (The C Speller)
            ├── main.c                  → Main entry point, event loop, Unity Build wrapper
            ├── main.py                 → Dareplane Python wrapper (compiles main.exe)
            ├── main.exe                → Compiled executable (generated at runtime)
            ├── modules/                → Pure C logic (fps.c, lsl.c, keyboard.c, tts.c, etc.)
            ├── codes/                  → m-sequences (.txt files only)
            ├── fonts/                  → UI Fonts (Montserrat, FontAwesome)
            ├── words/                  → en.txt (14,000 word dictionary)
            └── log.csv                 → Performance log (generated upon exit)
```
